...
active_profile: "profiles/agent-native.yaml"  # 変更するだけで他社に切替
...
5. 既存のストーリーライン（Kazuma 9パート構成等）があればそれを採用
...
**プロファイルに図解仕様を定義:**
```yaml
# profiles/agent-native.yaml の deck_types 内で指定
deck_types:
  company:
    diagrams:
      - slide: "p10"
...
└── profiles/
    ...
    └── agent-native.yaml     ← Agent Native用プロファイル
```
