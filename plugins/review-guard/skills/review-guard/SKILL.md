...
```jsonl
{"skill": "review-guard", "pattern": "{検出パターン}", "correction": "{自動修正内容 or エスカレーション結果}", "layer": "L3", "source": "{対象ファイルパス}"}
```
...
| test-runner | L3がテスト実行を依頼 |
| security-scan | セキュリティ違反検出時に連携 |
| rapid-build | Step 4（品質ゲート）と統合 |
