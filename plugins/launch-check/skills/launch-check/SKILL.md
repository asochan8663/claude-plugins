---
name: launch-check
description: "作業完了時の品質チェック→マージ→push。セキュリティ・テスト・承認ゲートを自動化。Quality gate before merge: auto-test, security-audit, approval, merge+push. 「launch-check」「ship」「完了」「マージ」「出荷」等で起動。"
triggers:
  - "launch-check"
  - "ship"
  - "完了"
...
**Configが空の場合**: auto-test → security-audit → ユーザー承認 → merge + push。追加チェック・タスク管理は全てスキップ。
...
auto-testスキルを呼び出す（auto-testがインストールされている場合）:
- テストフレームワークの自動検出
- テスト実行 + 結果レポート
...
security-auditスキルを呼び出す（security-auditがインストールされている場合）。

security-auditがない場合、軽量スキャンを実行:
```bash
# シークレットパターン検出
...
| auto-test | Step 2 | テスト実行をauto-testに委譲（インストール済みの場合） |
| security-audit | Step 3 | セキュリティ検証をsecurity-auditに委譲（インストール済みの場合） |
| safety-net | 連携 | safety-netがインストール済みならStep 2で品質検証も実行 |
