...
```
単一: skill-upgrade .claude/skills/auto-test/
複数: skill-upgrade .claude/skills/auto-test/ .claude/skills/slack-brain/
...
実行:
  1. Group Aを並列エージェントで処理（multi-agentスキル利用）
  2. Group Bを並列処理
  3. Group Cを並列処理
  4. 全スキルの最終検証
```
...
| multi-agent | 一括適用時の並列実行 | --allモードで23スキルを並列処理 |
| security-audit | Q3の参照先 | セキュリティセクションの詳細基準を参照 |
