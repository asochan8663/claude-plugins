...
### Step 1.5: 命名設計（BLOCKING）

`references/NAMING_GUIDE.md` をReadし、以下を実行する:

1. 機能を1文で書く
2. 動詞-名詞で候補を3つ出す
3. 衝突チェック（npm, GitHub Actions, claude-plugins-official, Trail of Bits）
4. 10秒テスト（名前だけ見て機能を推測できるか）
5. 決定

**リネーム時**: NAMING_GUIDE.mdの「リネーム時の追加手順」に従い、旧名の全参照を検索→置換。
